...
export async function exportToPDF(cases: VetCase[], title = 'SurgiCase Report'): Promise<void> {
    ...
    for (let i = 0; i < cases.length; i++) {
        ...
        const details = [
            [`Owner: ${c.ownerLastName}`, `Species: ${SPECIES_LABELS[c.species] || c.species}`],
            [`Breed: ${c.breed}`, `Sex: ${SEX_LABELS[c.sex] || c.sex}`],
            [`Arrival: ${c.arrivalDate}`, `DOB: ${c.dateOfBirth}  Age: ${calculateAge(c.dateOfBirth)}`],
        ];
        ...
        if (c.presentingComplaint) {
            ...
            doc.setFont('helvetica', 'normal');
            const lines = doc.splitTextToSize(c.presentingComplaint, contentWidth - 25);
