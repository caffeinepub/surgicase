import type { Sex, Species, VetCase } from "../types/case";
...
export function parseQuickFill(text: string): Partial<VetCase> {
  ...
  const mrnMatch = text.match(
    /(?:MRN[:\s#]*|Medical\s+Record\s*#?\s*:?\s*|#\s*)([A-Za-z0-9-]+)/i,
  );
  ...
  const arrivalMatch = text.match(
    /(?:Arrival\s*Date|Arrived)[:\s]+(\d{1,2}\/\d{1,2}\/\d{4})/i,
  );
  ...
  const petMatch = text.match(
    /(?:Patient|Pet|Name)[:\s]+([A-Za-z][A-Za-z\s'-]*?)(?:\s*[,\n]|$)/i,
  );
  if (petMatch) result.petName = petMatch[1].trim();

  // Owner last name
  const ownerMatch = text.match(
    /(?:Owner|Last\s+Name)[:\s]+([A-Za-z][A-Za-z\s'-]*?)(?:\s*[,\n]|$)/i,
  );
  if (ownerMatch) result.ownerLastName = ownerMatch[1].trim();

  // Species
  if (/\b(dog|canine)\b/i.test(text)) result.species = "canine" as Species;
  else if (/\b(cat|feline|kitty)\b/i.test(text))
    result.species = "feline" as Species;
  else if (/\b(other|rabbit|bird|reptile|exotic)\b/i.test(text))
    result.species = "other" as Species;

  // Breed
  const breedMatch = text.match(
    /(?:Breed)[:\s]+([A-Za-z][A-Za-z\s'-]*?)(?:\s*[,\n]|$)/i,
  );
  ...
  const complaintMatch = text.match(
    /(?:Complaint|CC|Presenting|Chief\s+Complaint)[:\s]+(.+?)(?:\n|$)/i,
  );
